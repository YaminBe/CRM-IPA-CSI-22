    ...
    <section class="inner-page">
      <div class="container">
        ...
        if (empty($_SESSION['startResponden'])) {
          ...
          Kuesioner ini merupakan instrument dalam penelitian yang berjudul <b>Custumer Relationship Management (Crm) Peningkatan Penjualan Gorden Berbasis Web Pada Usi-Usi Interior Bukittinggi Menggunakan Metode <em>Importance Performance Analysis</em> (IPA) dan <em>Customer Satisfaction Index</em> (CSI)</b>, guna penyelesaian tugas praktek kerja lapangan (pkl) Jurusan Sistem Informasi Fakultas Ilmu Komputer Universitas Putra Indonesia “Yptk” Padang, yang dilakukan oleh :
          <ul>
          <li> Nama    : Reski Andrian   18101152610662</li>
          <li>P.Oscar Sitanggang  18101152610657</li>
          </ul>
          Saya mohon kesedian Bapak/Ibuk dan saudara untuk mengisi kuesioner ini secara lengkap. Informasi yang diterima dari hasil kuesioner ini bersifat rahasia dan dipergunakan untuk kepentingan akademis. <br> Atas perhatian dan kerjasamanya saya ucapkan terimakasih. 
          </p>
          </div>
          </div>
          </div>
...
<form method="post">
    ...
    <div class="col-sm-4">
    <input type="text" name="nama" class="form-control" id="nama" maxlength="45" placeholder="Contoh : Budi Utama" required>
    </div>
    </div>
...
<div class="col-sm-4">
<input type="number" name="umur" class="form-control" id="Usia" placeholder="Contoh : 25" required>
</div>
</div>
...
<button type="submit" name="onuser" class="btn btn-primary"><i class="bi bi-arrow-right"></i> Lanjutkan</button>
</div>
</div>
</div>
...
<div class="alert alert-success intruksi">
  <p>
    Di bawah ini terdapat pertanyaan-pertanyaan yang berkaitan dengan tingkat kepentingan dan kinerja menurut menurut persepsi anda sebagai pelanggan Usi-Usi Interior Bukittinggi.
     Dimohon anda memberikan penilaian dengan tanda check pada kotak-kotak pertanyaan tersebut.
     <ul>
       <li> Tingkat kenyataan (kinerja yang diterima) menyatakan seberapa puas layanan yang sudah anda terima. </li>
...
<div class="table-responsive">
      ...
      <th scope="col" ><?php if ($cekKepuasan< 1) {echo 'Kepuasan';}else{echo 'Kepentingan';}?>
       </th>
      ...
      <input name="puas-<?= $i ?>" class="form-check-input" type="checkbox" id="p-<?= $i ?>" value="1">
      <label class="form-check-label" for="p-<?= $i ?>">1</label>
      </div>
      <div class="form-check form-check-inline">
      <input name="puas-<?= $i ?>" class="form-check-input" type="checkbox" id="p-<?= $i ?>" value="2">
      <label class="form-check-label" for="p-<?= $i ?>">2</label>
      </div>
      <div class="form-check form-check-inline">
      <input name="puas-<?= $i ?>" class="form-check-input" type="checkbox" id="p-<?= $i ?>" value="3">
      <label class="form-check-label" for="p-<?= $i ?>">3</label>
      </div>
      <div class="form-check form-check-inline">
      <input name="puas-<?= $i ?>" class="form-check-input" type="checkbox" id="p-<?= $i ?>" value="4">
      <label class="form-check-label" for="p-<?= $i ?>">4</label>
      </div>
      <div class="form-check form-check-inline">
      <input name="puas-<?= $i ?>" class="form-check-input" type="checkbox" id="p-<?= $i ?>" value="5">
...
<tr>
  <td colspan="4" align="center">
        ...
        ?>
        <button type="submit" name="submit" class="btn btn-success" onkeyup="return confirm('Apakah Anda Yakin Untuk Melanjutkan ? Selanjutnya akan diarah ke pengisian Tingkat Kepentingan, Klik OK Jika Iya')"> <i class="bi bi-arrow-right"></i> Kirim & Lanjutkan</button>
        <?php
      }else{
         ?>
        <button type="submit" name="submitKepentingan" class="btn btn-success" onkeyup="return confirm('Apakah Anda Yakin ? Jawaban Ini akan dikirim ')"><i class="bi bi-send"></i> Submit</button>
        <?php
      }?>
  </td>
</tr>
